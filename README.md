# Olá, eu sou o Diogo Borges de Moura 👋

🌐 Scroll down to keep reading this README in English or Click here for Pt-br version --> [Versão em Português](#-versão-em-português)

🔍 "Building scalable SaaS solutions, data-driven dashboards, and real-time IoT architectures."
📌 Currently: Final-year Computer Science Student @ CEUB
🌱 Focus: Full-Stack Backend + Business Intelligence & Data Pipelines + IoT Integrations


## 🛠️ Tech Stack

| Domain | Technologies & Tools |
| :--- | :--- |
| **Backend & Systems** | Python, JavaScript / Node.js, C++, PostgreSQL, MySQL, REST APIs |
| **Data & BI** | Power BI, DAX, Data Modeling, ETL Pipelines, SQL |
| **IoT & Embedded Systems** | ESP32, ESP8266, MQTT Protocol, Real-time Communication, Hardware Automation |
| **DevOps & Infrastructure** | Linux (Ubuntu, Kali), Docker, Git, CI/CD, Server Management |

---

## 🚀 Highlighted Projects

### 🏘️ CondoMovel: Smart Condominium Management Platform
* **Scope:** Full-stack SaaS platform designed for modern property and condominium management.
* **Architecture:** Integrated IoT microcontrollers (ESP32) for real-time automated gate access, combined with a robust backend service.
* **Key Achievements:** End-to-end relational database design, automated business rules, and hardware-software communication via MQTT.
* **Tech Stack:** Python, Node.js, PostgreSQL, ESP32, MQTT, Docker

### 📊 Enterprise BI & Data Automation
* **Scope:** Interactive Business Intelligence suite and automated data workflows for decision-making.
* **Pipeline:** ETL processes extracting from multiple relational databases to structured reporting models using advanced DAX.
* **Impact:** Streamlined IT monitoring metrics and business operational workflows into real-time decision-ready dashboards.
* **Tech Stack:** Power BI, DAX, SQL, Python, Linux Server Automation

---

## 🎓 Education & Background

* **B.S. in Computer Science** — *Centro Universitário de Brasília (CEUB)* (Expected Graduation: 2026)
* **Background:** Proven experience in Business Intelligence, IT Infrastructure Support, and Data Automation in corporate environments.

---

## 🌟 What Defines My Work

* **Hardware-to-Cloud Integration:** Ability to bridge physical IoT devices with modern cloud/web backends.
* **Data-Informed Systems:** Designing software architectures that seamlessly collect, process, and visualize decision-ready data.
* **Linux Rigor:** Strong foundation in environment configuration, deployment, and systems administration.

---

## 📫 Let's Connect

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/diogo-borges-de-moura-3b1b53289/) 
[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:diogodbm9@email.com)

---
---

# 🇧🇷 Versão em Português

🔍 "Construindo soluções SaaS escaláveis, dashboards orientados a dados e arquiteturas IoT em tempo real."
📌 Atualmente: Formando em Ciência da Computação @ CEUB
🌱 Foco: Desenvolvimento Backend Full-Stack + Business Intelligence & Pipelines de Dados + Integrações IoT

---

## 🛠️ Principais Habilidades & Tecnologias

| Domínio | Tecnologias & Ferramentas |
| :--- | :--- |
| **Backend & Sistemas** | Python, JavaScript / Node.js, C++, PostgreSQL, MySQL, APIs RESTful |
| **Dados & BI** | Power BI, DAX, Modelagem de Dados, Pipelines ETL, SQL |
| **IoT & Embarcados** | ESP32, ESP8266, Protocolo MQTT, Comunicação em Tempo Real, Automação |
| **DevOps & Infraestrutura** | Linux (Ubuntu, Kali), Docker, Git, CI/CD, Gerenciamento de Servidores |

---

## 🚀 Projetos Destacados

### 🏘️ CondoMovel: Plataforma Inteligente de Gestão Condominial
* **Escopo:** Plataforma SaaS Full-stack desenvolvida para automação e gestão moderna de condomínios.
* **Arquitetura:** Integração com microcontroladores (ESP32) para controle de acesso automatizado a portões em tempo real.
* **Resultados:** Modelagem de banco de dados relacional, regras de negócio automatizadas e ecossistema integrado via MQTT.
* **Tecnologias:** Python, Node.js, PostgreSQL, ESP32, MQTT, Docker

### 📊 BI Empresarial & Automação de Dados
* **Escopo:** Suíte de Business Intelligence e fluxos automatizados de dados para suporte à tomada de decisão.
* **Pipeline:** Processos de ETL extraindo de múltiplos bancos de dados relacionais para modelos estruturados com DAX avançado.
* **Impacto:** Otimização do monitoramento de métricas de TI e processos operacionais em dashboards em tempo real.
* **Tecnologias:** Power BI, DAX, SQL, Python, Automação Linux

---

## 🎓 Formação Acadêmica

* **Bacharelado em Ciência da Computação** — *Centro Universitário de Brasília (CEUB)* (Conclusão prevista: 2026)
* **Experiência:** Vivência prática com Business Intelligence, infraestrutura/suporte em ambiente Linux e automação de processos de TI.

---

## 🌟 O Que Define Meu Trabalho

* **Integração Hardware-to-Cloud:** Capacidade de conectar dispositivos IoT físicos a backends web modernos.
* **Sistemas Guiados a Dados:** Arquiteturas projetadas para coletar, processar e transformar dados brutos em insights práticos.
* **Domínio em Ambientes Linux:** Base sólida no gerenciamento de servidores, configurações e implantação de sistemas.

---

## 📫 Vamos Conversar

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/diogo-borges-de-moura-3b1b53289/) 
[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:seu-email@email.com)
